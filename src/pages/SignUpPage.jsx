  ...
  if (done) return (
    <div style={{ minHeight: '100vh', background: 'var(--bg-base)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <div style={{ textAlign: 'center', maxWidth: 380 }}>
        ...
        <h2 style={{ fontFamily: 'var(--font-serif)', fontSize: 22, fontWeight: 500, marginBottom: 10 }}>Check your email</h2>
        ...
        <Link to="/login" style={{ color: 'var(--accent)', fontWeight: 500, fontSize: 14 }}>← Back to sign in</Link>
      </div>
    </div>
  )
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg-base)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ width: '100%', maxWidth: 380, position: 'relative' }}>
        <div style={{ textAlign: 'center', marginBottom: 32 }}>
          ...
          <p style={{ fontSize: 13, color: 'var(--text-muted)', margin: 0 }}>Create your account</p>
        </div>

        <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 24, padding: 32, boxShadow: 'var(--shadow-lg)' }}>
          <h2 style={{ fontFamily: 'var(--font-serif)', fontSize: 18, fontWeight: 500, margin: '0 0 22px' }}>Get started</h2>
          ...
          <form onSubmit={handleSubmit} noValidate>
            {[
              { id: 'firstName',    label: 'First name',       type: 'text',     val: firstName, set: setFirstName, ph: '' },
              { id: 'lastName',     label: 'Last name',        type: 'text',     val: lastName,  set: setLastName,  ph: '' },
              ...
              { id: 'confirm',  label: 'Confirm password', type: 'password', val: confirm,  set: setConfirm,  ph: '••••••••' },
            ].map(f => (
              <div key={f.id} style={{ marginBottom: 16 }}>
                <label htmlFor={f.id} style={{ display: 'block', fontSize: 12, fontWeight: 500, marginBottom: 5, color: 'var(--text-secondary)' }}>
                  {f.label}
                </label>
                <input
                  id={f.id}
                  ...
                  placeholder={f.ph}
                  required
                  autoComplete={f.id === 'email' ? 'email' : f.id === 'password' ? 'new-password' : 'new-password'}
                  style={inputStyle}
