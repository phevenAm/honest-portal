...
const exportClientPDF = async (user: any, responses: any[]) => {
  ...
  if (responses.length > 0) {
    const avgScores = responses.map(r =>
      parseFloat((Object.values(r.scores as Record<string, number>).reduce((a, b) => a + b, 0) / 4).toFixed(1))
    );
    ...
    [[`${overall}/10`, 'Overall avg'], [`${latest}/10`, 'Latest'], [`${parseFloat(change) >= 0 ? '+' : ''}${change}`, 'Change']].forEach(([val, label], i) => {
...
function AddClientModal({ onClose }: { onClose: () => void }) {
  ...
  const handleSubmit = () => {
    if (!form.name.trim() || !form.email.trim()) { setError('Both fields are required'); return; }
...
function ClientRow({ user }: { user: any }) {
  ...
  const avgScore = latest
    ? (Object.values(latest.scores as Record<string, number>).reduce((a, b) => a + b, 0) / 4).toFixed(1)
    : null;
    ...
    <>
      <div className={styles.clientRow}>
        ...
        <div className={styles.clientMeta}>
          <p className={styles.clientName}>{user.first_name} {user.last_name}</p>
          <p className={styles.clientEmail}>{user.email}</p>
        </div>
        <div className={styles.statBlock}>
          <p className={styles.statValue}>{avgScore ?? '–'}<span>/10</span></p>
        ...
        <div className={styles.rowActions}>
          ...
          <Button
            ...
            onClick={() => {
              if (window.confirm(`Remove ${user.first_name} ${user.last_name} from the portal?`)) {
      ...
      {expanded && (
        <div className={styles.expandedChart}>
          <ProgressChart responses={responses} title={`${user.first_name}'s Progress`} />
        </div>
      )}
    </>
  );
    ...
    <div className={styles.page}>
      <div className={styles.inner}>
        <div className={styles.pageHeader}>
          <div>
            <h1>Clients</h1>
            <p>{allClients.length} active {allClients.length === 1 ? 'client' : 'clients'}</p>
          </div>
