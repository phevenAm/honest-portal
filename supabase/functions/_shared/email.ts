...
export function emailTemplate({ label, title, body, cta, footerNote }: EmailTemplateOptions): string {
  const ctaBlock = cta
...
<div style="background:#f3f0eb;padding:40px 20px;font-family:Georgia,serif;">
  <div style="max-width:560px;margin:0 auto;">
    ...
    <div style="background:#2d2926;border-radius:0 0 16px 16px;padding:28px 40px;">
      ...
      <p style="font-family:system-ui,sans-serif;font-size:12px;color:#706c68;line-height:1.7;margin:0 0 12px;">${footerNote}</p>
...
export async function sendEmail(opts: {
  ...
  const res = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({ from: opts.fromEmail, to: opts.to, subject: opts.subject, html: opts.html }),
  });
  if (!res.ok) throw new Error(`Resend error: ${await res.text()}`);
}

/** Formats an ISO date string for UK display */
